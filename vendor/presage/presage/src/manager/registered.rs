...
impl<S: Store> Manager<S, Registered> {
    /// Find a contact by phone number in the local store.
    pub async fn contact_by_phone_number(
        &self,
        phone_number: &PhoneNumber,
    ) -> Result<Option<Contact>, Error<S::Error>> {
        let contacts = self.store.contacts().await?;
